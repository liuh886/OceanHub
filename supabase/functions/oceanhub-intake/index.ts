...
Deno.serve(async (request) => {
  ...
  if (action === 'create_request') {
    ...
    const normalizedProviders = providerRequests.map((raw) => {
      ...
      return {
        providerId: cleanId(item.providerId),
        providerName: cleanText(item.providerName, 240),
        capabilityIds: uniqueStrings((Array.isArray(item.capabilityIds) ? item.capabilityIds : []).map(cleanId)).filter(Boolean)
      };
    });
    ...
    const providersValid = normalizedProviders.every((provider) =>
      provider.providerId && provider.providerName && provider.capabilityIds.length > 0 && provider.capabilityIds.every((capabilityId) => requiredSet.has(capabilityId))
    );
    ...
    const { data, error } = await admin.from('oceanhub_eoi_requests').insert({
  ...
  if (action === 'submit_response') {
    ...
    const notClaimedCapabilityIds = uniqueStrings((Array.isArray(body.notClaimedCapabilityIds) ? body.notClaimedCapabilityIds : []).map(cleanId)).filter(Boolean);
    ...
    const { data: eoi, error: eoiError } = await admin.from('oceanhub_eoi_requests').select('id,provider_requests').eq('id', requestId).single();
    if (eoiError || !eoi) return jsonResponse(origin, 404, { error: 'EOI request not found.' });
    ...
    const requestedCapabilityIds = providerRequest && Array.isArray(providerRequest.capabilityIds)
      ...
      : [];
    if (!providerRequest || requestedCapabilityIds.length === 0) return jsonResponse(origin, 400, { error: 'Provider is not part of this EOI request.' });
    ...
    const claimedCapabilityIds = uniqueStrings(normalizedAssertions.map((item) => item.capabilityId)).filter(Boolean);
    const combinedIds = uniqueStrings([...claimedCapabilityIds, ...notClaimedCapabilityIds]);
    const noOverlap = claimedCapabilityIds.every((id) => !notClaimedCapabilityIds.includes(id));
    const assertionsValid = normalizedAssertions.every((item) => item.capabilityId && item.capabilityLabel && evidenceTypes.has(item.evidenceType) && item.evidenceDetail);

    if (!assertionsValid || !noOverlap || !sameSet(combinedIds, requestedCapabilityIds)) {
      return jsonResponse(origin, 400, { error: 'Every requested capability must be either claimed with evidence or explicitly not claimed.' });
    }

    const now = new Date().toISOString();
    const { data: response, error: responseError } = await admin.from('oceanhub_supplier_responses').upsert({
      eoi_request_id: requestId,
      provider_id: providerId,
      organization,
      contact_email: contactEmail,
      operating_region: operatingRegion,
      not_claimed_capability_ids: notClaimedCapabilityIds,
      updated_at: now
    }, { onConflict: 'eoi_request_id,provider_id' }).select('id').single();
    ...
    if (normalizedAssertions.length) {
      ...
      const { error: assertionsError } = await admin.from('oceanhub_capability_assertions').upsert(rows, { onConflict: 'id' });
      if (assertionsError) {
    ...
    const staleIds = requestedCapabilityIds.filter((capabilityId) => !claimedCapabilityIds.includes(capabilityId));
    if (staleIds.length) {
      await admin.from('oceanhub_capability_assertions').delete().eq('eoi_request_id', requestId).eq('provider_id', providerId).in('capability_id', staleIds);
    }
  ...
  if (action === 'request_status') {
    ...
    if (!requestId || !buyerToken) return jsonResponse(origin, 400, { error: 'Request ID and buyer token are required.' });

    const { data: eoi, error: eoiError } = await admin.from('oceanhub_eoi_requests')
      .select('id,project_context,lifecycle_stage,decision_focus,provider_requests,created_at')
      .eq('id', requestId).eq('buyer_token', buyerToken).single();
    if (eoiError || !eoi) return jsonResponse(origin, 404, { error: 'Tracked EOI request not found.' });

    const { data: responses } = await admin.from('oceanhub_supplier_responses')
      .select('id,provider_id,not_claimed_capability_ids,submitted_at').eq('eoi_request_id', requestId);
    const { data: assertions } = await admin.from('oceanhub_capability_assertions')
      .select('provider_id,capability_id,capability_label,review_state,review_outcome,reviewed_at').eq('eoi_request_id', requestId);

    const responseByProvider = new Map((responses ?? []).map((response) => [response.provider_id, response]));
    const assertionsByProvider = new Map<string, NonNullable<typeof assertions>>();
    ...
    const providers = (Array.isArray(eoi.provider_requests) ? eoi.provider_requests as Array<Record<string, unknown>> : []).map((item) => {
      ...
      const response = responseByProvider.get(providerId);
      return {
        providerId,
        providerName: cleanText(item.providerName, 240),
        requestedCapabilityIds: Array.isArray(item.capabilityIds) ? item.capabilityIds.map(cleanId).filter(Boolean) : [],
        responseState: response ? 'evidence-submitted' : 'awaiting-response',
        submittedAt: response?.submitted_at ?? null,
        notClaimedCapabilityIds: response?.not_claimed_capability_ids ?? [],
